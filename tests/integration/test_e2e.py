...
async def wait_for_service(url: str, timeout: int = 30):
    ...
    raise TimeoutError(
        f"Service {url} did not become available within {timeout} seconds."
    )


async def ingest_test_document(
    client: httpx.AsyncClient, doc_content: str, metadata: dict[str, Any]
) -> dict[str, Any]:
...
async def search_vector(
    ...
    response = await client.post(
        f"{BASE_URL}/api/v1/search/chunks/vector", json={"query": query, "limit": limit}
    )
    response.raise_for_status()
    return response.json()
...
async def clean_db_e2e() -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        if driver:
            await driver.close()
            logger.info("[Fixture] Closed DB connection after test.")
...
async def test_ingest_and_keyword_search(integration_test_client: httpx.AsyncClient):
    ...
    if ingest_response.status_code != status.HTTP_202_ACCEPTED:
        print(
            f"Ingest response status: {ingest_response.status_code}, body: {ingest_response.text}"
        )
    ...
    if search_response.status_code != status.HTTP_200_OK:
        print(
            f"Search response status: {search_response.status_code}, body: {search_response.text}"
        )
    ...
    found = any(
        res.get("chunk", {}).get("document_id") == doc_id
        for res in search_results["results"]
    )
    assert found, (
        f"No chunk from document {doc_id} found in vector search results for '{search_query}'"
    )
...
async def test_ingest_and_vector_search(integration_test_client: httpx.AsyncClient):
    ...
    if search_response.status_code != status.HTTP_200_OK:
        print(
            f"Vector search response status: {search_response.status_code}, body: {search_response.text}"
        )
    assert search_response.status_code == status.HTTP_200_OK
    ...
    assert any(
        "cars" in res["chunk"]["text"] or "vehicles" in res["chunk"]["text"]
...
async def test_search_nonexistent_term(integration_test_client: httpx.AsyncClient):
    ...
    if keyword_response.status_code != status.HTTP_200_OK:
        print(
            f"Keyword search response status: {keyword_response.status_code}, body: {keyword_response.text}"
        )
    ...
    assert not any(
        query in res["chunk"]["text"] for res in keyword_results["results"]
    ), (
    ...
    if vector_response.status_code != status.HTTP_200_OK:
        print(
            f"Vector search response status: {vector_response.status_code}, body: {vector_response.text}"
        )
    assert vector_response.status_code == status.HTTP_200_OK
    vector_results = vector_response.json()
    assert "results" in vector_results
    assert not any(
        query in res["chunk"]["text"] for res in vector_results["results"]
...
async def test_ingest_and_delete(integration_test_client: httpx.AsyncClient):
    ...
    while time.time() - start_time < max_wait_seconds:
        delete_response = await integration_test_client.delete(
            f"{BASE_URL}/api/v1/documents/{doc_id}"
        )
        if delete_response.status_code == status.HTTP_204_NO_CONTENT:
            logger.info(
                f"Document {doc_id} deleted successfully after {time.time() - start_time:.2f} seconds."
            )
            delete_successful = True
            break
        elif delete_response.status_code == status.HTTP_404_NOT_FOUND:
            logger.debug(
                f"Document {doc_id} not found yet for deletion, polling... (Status: {delete_response.status_code})"
            )
        else:
            # Unexpected status code during deletion attempt
            print(
                f"Unexpected delete response status: {delete_response.status_code}, body: {delete_response.text}"
            )
            pytest.fail(
                f"Unexpected status code {delete_response.status_code} during deletion poll for {doc_id}"
            )
        await asyncio.sleep(poll_interval)

    if not delete_successful:
        pytest.fail(
            f"Document {doc_id} was not successfully deleted within {max_wait_seconds} seconds."
        )
    ...
    while time.time() - start_time_search < max_wait_seconds:
        ...
        if keyword_response.status_code != status.HTTP_200_OK:
            ...
            pytest.fail(
                f"Keyword search failed with status {keyword_response.status_code} during verification."
            )

        keyword_results = keyword_response.json()
        if len(keyword_results.get("results", [])) == 0:
            logger.info(
                f"Search verification successful (0 results) after {time.time() - start_time_search:.2f} seconds."
            )
            search_verified = True
            break
        else:
            logger.debug(
                f"Search verification found {len(keyword_results.get('results', []))} results, polling... Results: {keyword_results.get('results')}"
            )

        await asyncio.sleep(poll_interval)

    assert search_verified, (
        f"Search still returned results for deleted doc {doc_id} after {max_wait_seconds} seconds."
    )
...
async def test_delete_nonexistent(integration_test_client: httpx.AsyncClient):
    ...
    delete_response = await integration_test_client.delete(
        f"{BASE_URL}/api/v1/documents/{non_existent_id}"
    )  # Adjust endpoint if needed
    if delete_response.status_code != status.HTTP_404_NOT_FOUND:
        print(
            f"Delete response status: {delete_response.status_code}, body: {delete_response.text}"
        )
