...
def mock_document_processor(mocker):  # Corrected fixture name
    ...
    mock.chunk_document.return_value = [
        ...
        ChunkData(
            id="test-chunk-2",
            text="It was designed by Gustave Eiffel and completed in 1889.",
            document_id="test-doc",
        ),
        ChunkData(
            id="test-chunk-3",
            text="The tower is 330 meters tall and is the most-visited paid monument in the world.",
            document_id="test-doc",
        ),
    ]
    return mock
...
async def test_ingest_document_cli_logic(  # Rename test for clarity
    ...
    try:
        ...
        await ingest(file_path=file_path, metadata=json.dumps(test_metadata))
        ...
        assert result is not None and len(result) == 1, (
            f"Document with source '{test_metadata['source']}' not found."
        )
        ...
        chunk_result = await memgraph_repo.execute_query(
            chunk_check_query, {"doc_id": doc_id}
        )
        ...
        """
        result_entities = await memgraph_repo.execute_query(
            entity_check_query, {"doc_id": doc_id}
        )
        ...
        if "doc_id" in locals() and doc_id:
            ...
            logger.warning(
                f"Could not retrieve doc_id, attempting cleanup by metadata: {find_doc_params}"
            )
            # This might still fail if add_document failed initially
            # Find ID first, then delete
            fallback_result = await memgraph_repo.execute_query(
                "MATCH (d:Document {source: $source}) RETURN d.id as id LIMIT 1",
                find_doc_params,
            )
            if fallback_result and fallback_result[0].get("id"):
                fallback_doc_id = fallback_result[0]["id"]
                logger.info(
                    f"Attempting fallback cleanup for doc_id: {fallback_doc_id}"
                )
                await memgraph_repo.delete_document(fallback_doc_id)
            else:
                logger.error(
                    f"Fallback cleanup failed: Could not find document by metadata {find_doc_params}"
                )


@pytest.mark.asyncio
async def test_ingest_single_file_success(
    memgraph_repo: MemgraphGraphRepository, mock_document_processor, tmp_path
):
...
@pytest.mark.asyncio
async def test_ingest_directory_success(
    memgraph_repo: MemgraphGraphRepository, mock_document_processor, tmp_path
):
...
async def test_ingest_file_not_found(
    memgraph_repo: MemgraphGraphRepository, mock_document_processor
):
...
@pytest.mark.asyncio
async def test_ingest_processing_error(
    memgraph_repo: MemgraphGraphRepository, mock_document_processor, tmp_path
):
...
@pytest.mark.asyncio
async def test_ingest_repository_error(
    memgraph_repo: MemgraphGraphRepository, mock_document_processor, tmp_path, mocker
):
