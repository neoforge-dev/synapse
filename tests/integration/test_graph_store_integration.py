...
class TestMemgraphGraphRepositoryIntegration:
    ...
    async def repo(
        self, memgraph_connection
    ) -> AsyncGenerator[MemgraphGraphRepository, None]:
        ...
        try:
            ...
            logger.error(
                f"Failed to clean database in repo fixture setup: {e}", exc_info=True
            )
    ...
    async def test_document_operations(self, repo):
        ...
        chunk_data_dict["metadata"] = chunk_data_dict.get(
            "metadata", None
        ...
        assert retrieved_node is not None, (
            f"Chunk {chunk.id} not found using get_node_by_id"
        )
        assert isinstance(retrieved_node, Chunk), (
            f"Node {chunk.id} is not a Chunk object"
        )
        ...
        if chunk.embedding:
            assert np.allclose(retrieved_node.embedding, chunk.embedding), (
                "Chunk embeddings do not match"
            )
    ...
    async def test_entity_operations(self, repo):
        ...
        neighbors, relationships = await repo.get_neighbors(
            entity1.id, relationship_types=["RELATED_TO"], direction="outgoing"
        )
    ...
    async def test_multiple_operations(self, repo):
        ...
        for chunk_data_dict in SAMPLE_CHUNKS:
            ...
            chunk_model_dict["metadata"] = chunk_model_dict.get(
                "metadata", None
            )  # Ensure metadata exists
        ...
        for chunk in chunks_to_add:
            ...
            if parent_doc:
                ...
                logger.warning(
                    f"Skipping chunk {chunk.id} because parent doc {chunk.document_id} not found"
                )
        ...
        for chunk in chunks_to_add:
            ...
            assert retrieved_chunk is not None, (
                f"Chunk {chunk.id} not retrieved after bulk add."
            )
    ...
    async def test_error_handling(self, repo):
        ...
        with pytest.raises(
            ValueError, match="Document with ID nonexistent_doc not found."
        ):
            await repo.add_chunk(chunk)
        ...
        try:
            ...
            pytest.fail(
                f"Adding relationship with non-existent nodes raised an unexpected exception: {e}"
            )
