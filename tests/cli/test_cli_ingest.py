...
def test_ingest_file_success(cli_runner: CliRunner, tmp_path: Path):
    ...
    with (
        ...
        assert result.exit_code == 0, (
            f"CLI command failed unexpectedly: {result.stdout}"
        )
...
def test_ingest_file_not_found(cli_runner: CliRunner, tmp_path: Path):
    ...
    with patch("graph_rag.cli.commands.ingest.typer.echo") as mock_echo:
        ...
        print(
            f"CLI Runner Result (Ingest Not Found Test):\nExit Code: {result.exit_code}\nOutput:\n{result.stdout}\nException:\n{result.exception}"
        )
...
def test_ingest_directory_success(cli_runner: CliRunner, tmp_path: Path):
    ...
    with (
        ...
        print(
            f"CLI Runner Result (Ingest Dir - File 1):\nExit Code: {result.exit_code}\nOutput:\n{result.stdout}\nException:\n{result.exception}"
        )
        ...
        # Check successful output message
        mock_echo.assert_any_call(
            f"Successfully processed and stored {file1} including graph links."
        )
