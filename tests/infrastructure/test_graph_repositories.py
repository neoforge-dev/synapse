...
from graph_rag.infrastructure.graph_stores.memgraph_store import MemgraphGraphRepository, MemgraphConnectionConfig
from graph_rag.infrastructure.repositories.graph_repository import MemgraphRepository
from graph_rag.domain.models import Entity, Relationship, Document, Chunk
...
def test_memgraph_repository_has_required_methods():
    ...
    graph_repo_methods = [name for name, _ in inspect.getmembers(GraphRepository, inspect.isfunction)]
    graph_store_methods = [name for name, _ in inspect.getmembers(GraphStore, inspect.isfunction)]
    
    # Get the list of methods implemented by MemgraphGraphRepository
    memgraph_repo_methods = [name for name, _ in inspect.getmembers(MemgraphGraphRepository, inspect.isfunction)]
    
    # Check if all required methods are implemented
    for method in graph_repo_methods:
        if method.startswith('_'): continue  # Skip private methods
        assert method in memgraph_repo_methods, f"MemgraphGraphRepository missing {method} from GraphRepository"
    
    # Check for GraphStore methods too
    for method in graph_store_methods:
        if method.startswith('_'): continue  # Skip private methods
        assert method in memgraph_repo_methods, f"MemgraphGraphRepository missing {method} from GraphStore"
...
async def test_memgraph_repository_methods():
    ...
    with patch('graph_rag.infrastructure.graph_stores.memgraph_store.MemgraphConnectionConfig'), \
        ...
        with patch.object(repo, 'execute_query', new_callable=AsyncMock) as mock_execute:
            ...
            with patch.object(repo, 'add_node', AsyncMock()) as mock_add_node, \
                 patch.object(repo, 'add_relationship', AsyncMock()) as mock_relationship, \
                 patch.object(repo, 'get_node_by_id', AsyncMock()) as mock_get_node, \
                 patch.object(repo, 'apply_schema_constraints', AsyncMock()) as mock_schema, \
                 patch.object(repo, 'detect_communities', AsyncMock()) as mock_communities, \
                ...
                await repo.add_relationship("source-id", "target-id", "TEST_REL", {"prop": "value"})
                await repo.get_node_by_id("test-node-id")
                await repo.apply_schema_constraints()
                await repo.detect_communities(algorithm="louvain", write_property="community")
