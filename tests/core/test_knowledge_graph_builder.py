...
from graph_rag.models import Entity, ProcessedDocument, Relationship
...
def sample_doc_kg() -> DocumentData:
    return DocumentData(
        id=DOC_ID_1, content="Apple was founded by Steve Jobs.", metadata={"year": 1976}
    )
...
def sample_chunk_kg() -> ChunkData:
    return ChunkData(
        id=CHUNK_ID_1, text="Apple was founded by Steve Jobs.", document_id=DOC_ID_1
    )
...
def sample_relationship_founded() -> ExtractedRelationship:
    return ExtractedRelationship(
        source_entity_id=ENTITY_ID_STEVE,
        target_entity_id=ENTITY_ID_APPLE,
        label="FOUNDED",
    )
...
def sample_processed_doc() -> ProcessedDocument:
    ...
    chunk1 = Chunk(
        id="c1", text="Alice works at Acme Corp. Alice knows Bob.", document_id="doc-1"
    )
...
async def test_add_chunk(
    memory_kg: InMemoryKnowledgeGraphBuilder, sample_chunk_kg, sample_doc_kg
):
...
async def test_add_entity(
    memory_kg: InMemoryKnowledgeGraphBuilder, sample_entity_apple
):
...
async def test_add_relationship(
    memory_kg: InMemoryKnowledgeGraphBuilder,
    sample_entity_apple,
    sample_entity_steve,
    sample_relationship_founded,
):
    # Add entities first
    await memory_kg.add_entity(sample_entity_apple)
...
async def test_link_chunk_to_entities(
    ...
    await memory_kg.add_chunk(
        sample_chunk_kg
    )  # Assumes add_chunk handles adding to its dict
    await memory_kg.add_entity(sample_entity_apple)
    await memory_kg.add_entity(sample_entity_steve)
...
@pytest.mark.asyncio
async def test_add_duplicate_document(
    memory_kg: InMemoryKnowledgeGraphBuilder, sample_doc_kg
):
...
@pytest.mark.asyncio
async def test_simple_builder_build(
    mock_graph_store: AsyncMock, sample_processed_doc: ProcessedDocument
):
    ...
    mock_graph_store.add_entities_and_relationships.assert_awaited_once_with(
        sample_processed_doc.entities, sample_processed_doc.relationships
    )


@pytest.mark.asyncio
async def test_simple_builder_build_empty(
    mock_graph_store: AsyncMock, empty_processed_doc: ProcessedDocument
):
...
async def test_inmemory_add_relationship():
    ...
    rel_data = ExtractedRelationship(
        source_entity_id="ent1", target_entity_id="ent2", label="KNOWS"
    )
