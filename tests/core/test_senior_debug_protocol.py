...
def debug_protocol(request):
    ...
    return SeniorDebugProtocol(
        test_file=relative_test_file, test_function=test_function_name
    )
...
ValueError: Test error message"""
    ...
    assert all(
        "file" in step and "line" in step and "function" in step for step in path
    )
...
def test_save_and_load_investigation(
    debug_protocol: SeniorDebugProtocol, sample_error, tmp_path: Path
):
