...
if not check_memgraph_connection():
    pytest.skip(
        "Cannot connect to Memgraph, skipping all MVP E2E tests.",
...
def mock_embedding_service():
    ...
    with patch(
        "graph_rag.services.embedding.EmbeddingService", autospec=True
    ) as mock_emb_service:
        ...
        mock_emb_service._get_model.return_value = (
            MagicMock()
...
def test_mvp_ingest_and_query_flow(
    ...
    logger.info("E2E: Added chunks to vector store.")

    # Entity Extraction
    processed_doc: ProcessedDocument = entity_extractor.extract(
        doc
    )  # Extractor now works on Document
    ...
    logger.info(
        f"E2E: Extracted {len(processed_doc.entities)} entities: {entity_names}"
    )
    ...
    try:
        kg_builder.graph_store.add_entities_and_relationships(
            processed_doc.entities, []
        )
        logger.info(
            f"E2E: Added {len(processed_doc.entities)} entities to graph store."
        )
...
async def test_query_endpoint(client):
    ...
    client.application.extensions[
        "graph_store"
    ].session.return_value.run.return_value = mock_result
...
async def test_query_endpoint_with_entities(client):
    ...
    response = await client.post(
        "/query", json={"query": "Tell me about Alice and Bob"}
    )
...
async def test_query_endpoint_error_handling(client):
    ...
    client.application.extensions[
        "graph_store"
    ].session.return_value.run.side_effect = Exception("Test error")
...
async def test_e2e_mvp_pipeline(test_client: AsyncClient, memgraph_repo):
    ...
    doc_content = (
        f"E2E Test MVP {uuid.uuid4()}. Key entity: E2E_COMPANY. Another: E2E_PERSON."
    )
    ...
    assert "E2E_COMPANY" in query_result["answer"] or any(
        "E2E_COMPANY" in c.get("text", "")
        for c in query_result.get("relevant_chunks", [])
    ), "[E2E] Expected entity E2E_COMPANY not found in answer or relevant chunks."
    assert "E2E_PERSON" in query_result["answer"] or any(
        "E2E_PERSON" in c.get("text", "")
        for c in query_result.get("relevant_chunks", [])
    ...
    if query_result.get("graph_context"):
        ...
        assert "E2E_COMPANY" in entity_names, (
            "[E2E] E2E_COMPANY not found in graph context entities."
        )
        assert "E2E_PERSON" in entity_names, (
            "[E2E] E2E_PERSON not found in graph context entities."
        )

    print("[E2E] MVP Pipeline Test Completed Successfully.")
...
async def test_e2e_ingest_search_query(
    ...
    # --- 1. Ingest Document ---
    logger.info(
        f"E2E Test (ingest_search_query): Ingesting document {TEST_DOC_ID_E2E}..."
    )
    ingest_payload = {
    ...
    logger.info("E2E Test: Verified document exists in graph.")

    # --- 2. Search for Related Content ---
    logger.info("E2E Test: Searching for related content...")
    ...
    logger.info("E2E Test: Search returned chunks.")

    # --- 3. Query using RAG Engine ---
    logger.info("E2E Test: Querying with RAG engine...")
    ...
    query_response = await integration_test_client.post(
        "/api/v1/query/", json=query_payload
    )
    ...
    logger.info("E2E Test: RAG query successful.")

    # --- 4. Cleanup ---
    logger.info(f"E2E Test: Cleaning up document {TEST_DOC_ID_E2E}...")
    await cleanup_document(memgraph_repo, TEST_DOC_ID_E2E)
    logger.info("E2E Test: Completed successfully (test_e2e_ingest_search_query).")
