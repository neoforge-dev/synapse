...
def load_llm(
    ...
    config_llm_type = getattr(
        settings, "LLM_TYPE", "mock"
    )  # Safely get LLM_TYPE, default to mock
    ...
    if llm_class:
        try:
            ...
            logger.error(
                f"Failed to initialize LLM service '{llm_type_to_load}': {e}",
                exc_info=True,
            )
            raise ValueError(
                f"Failed to initialize LLM service '{llm_type_to_load}'."
            ) from e
    else:
        logger.error(
            f"Unsupported LLM type specified: '{llm_type_to_load}'. Available: {list(LLM_REGISTRY.keys())}"
        )
