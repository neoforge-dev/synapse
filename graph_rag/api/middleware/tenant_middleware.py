...
from ...architecture.multi_tenant.tenant_manager import (
    TenantManager, 
    TenantContext as NewTenantContext, 
    TenantStatus,
    get_tenant_manager
)
...
class EnhancedTenantMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        tenant_context: Optional[NewTenantContext] = None
    ...
    async def _log_request(
        ...
        request: Request, 
        response: Response, 
        tenant_context: Optional[NewTenantContext],
        duration: float
    ) -> None:
...
class TenantSecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        blocked_cross_tenant_paths: Optional[set] = None
    ):
...
def get_tenant_from_request(request: Request) -> Optional[NewTenantContext]:
