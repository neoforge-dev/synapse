...
class SSOProvider(BaseModel):
    ...
    oauth_authorization_url: Optional[HttpUrl] = None
    oauth_token_url: Optional[HttpUrl] = None
    oauth_userinfo_url: Optional[HttpUrl] = None
    oauth_scopes: Optional[List[str]] = Field(default_factory=lambda: ["openid", "profile", "email"])
    ...
    role_attribute: Optional[str] = Field(default="groups")
    role_mapping: Optional[Dict[str, UserRole]] = Field(default_factory=dict)
...
class EnterpriseSSOHandler:
    ...
    def generate_saml_auth_request(self, provider_id: str, relay_state: Optional[str] = None) -> tuple[str, str]:
...
<samlp:AuthnRequest
    ...
    def generate_oauth_auth_url(self, provider_id: str, state: Optional[str] = None) -> str:
    ...
    def create_sso_session(self, user: User, provider_id: str, external_user_id: str, 
                          saml_session_index: Optional[str] = None) -> SSOSession:
