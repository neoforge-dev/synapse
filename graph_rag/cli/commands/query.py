...
def make_api_request(url: str, method: str = "POST", payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Makes a request to the specified API endpoint and handles common errors."""
    logger.info(f"Sending {method} request to {url}...")
    try:
        with httpx.Client() as client:
            if method.upper() == "POST":
                response = client.post(url, json=payload, timeout=60.0) # Longer timeout for query
        ...
        response_text = "N/A"
        if 'exc' in locals() and hasattr(exc, 'response') and hasattr(exc.response, 'text'):
...
def ask_query(
    ...
    k: Optional[int] = typer.Option(None, "--k", help="Number of relevant chunks to retrieve (vector search)."),
    show_chunks: bool = typer.Option(False, "--show-chunks", "-c", help="Display the text of relevant chunks."),
    show_graph: bool = typer.Option(False, "--show-graph", "-g", help="Display the graph context (entities/relationships)."),
    raw: bool = typer.Option(False, "--raw", help="Output the raw JSON response from the API."),
    api_url: str = typer.Option(
        DEFAULT_QUERY_URL, 
        "--url", 
        help="URL of the query API endpoint."
    )
):
    ...
    console.print(Panel(response_data.get("answer", "No answer received."), title="[bold green]Answer[/bold green]"))
    ...
    if show_graph and response_data.get("graph_context"):
        ...
        if graph_context.get("relationships"):
            ...
            for rel in graph_context["relationships"]:
                 ...
                 source_id = rel.get('source', {}).get('id', 'N/A') if isinstance(rel.get('source'), dict) else rel.get('source_id', 'N/A')
                 target_id = rel.get('target', {}).get('id', 'N/A') if isinstance(rel.get('target'), dict) else rel.get('target_id', 'N/A')
                 rels_table.add_row(
                     source_id,
                     rel.get('type', 'N/A'), 
                     target_id,
