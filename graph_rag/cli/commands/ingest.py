...
from graph_rag.core.interfaces import ExtractedEntity, ExtractedRelationship, ExtractionResult # Import ExtractionResult too
...
async def process_and_store_document(
    ...
    ingestion_result = None # Initialize ingestion_result
    ...
    try:
        ...
        if not chunk_objects:
             logger.warning(f"No chunks found for document {document_id} after ingestion, skipping entity linking.")
        ...
        logger.info(f"Extracting entities from {len(chunk_objects)} chunks for doc {document_id}...")
        
        extraction_tasks = []
        chunk_map = {chunk.id: chunk for chunk in chunk_objects} # For associating results back to chunks
        for chunk_id, chunk in chunk_map.items():
            if chunk.text and not chunk.text.isspace():
                # Pass chunk_id for context/mapping
                extraction_tasks.append((chunk_id, extractor.extract_from_text(chunk.text)))
            else:
                logger.debug(f"Skipping empty chunk {chunk_id} for entity extraction.")

        if extraction_tasks:
            # Await all extraction tasks
            results_with_ids = await asyncio.gather(*[task[1] for task in extraction_tasks])
        ...
        logger.info(f"Extracted {len(final_entities)} unique entities for doc {document_id}.")
        ...
        for chunk_id, entity_ids in chunk_to_entity_ids.items():
             if entity_ids:
                 ...
                 logger.debug(f"Linking chunk {chunk_id} to entities: {unique_entity_ids}")
                 # Assume repo has link_chunk_to_entities method
                 link_tasks.append(repo.link_chunk_to_entities(chunk_id, unique_entity_ids))
                 
        if link_tasks:
             await asyncio.gather(*link_tasks)
             logger.info(f"Completed linking {len(chunk_to_entity_ids)} chunks to entities for doc {document_id}.")
        else:
...
async def ingest(
    ...
    try:
        ...
        typer.echo(f"Successfully processed and stored {file_path} including graph links.") 
    except Exception as e:
