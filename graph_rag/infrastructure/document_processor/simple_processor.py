...
class SentenceSplitter(ChunkSplitter):
    ...
    def _ensure_nltk_punkt(self):
        ...
        if not nltk:
            logger.warning(
                "'nltk' library not found. Sentence splitting will not work."
            )
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Error downloading NLTK 'punkt': {e}. Sentence splitting may fail.",
                    exc_info=True,
                )
    ...
    def split(self, document: Document) -> list[Chunk]:
        ...
        try:
            ...
            logger.error(
                f"NLTK sentence tokenization failed for doc {document.id}: {e}",
                exc_info=True,
            )
...
class SimpleDocumentProcessor(DocumentProcessor):
    ...
    def __init__(
        ...
        if chunk_strategy not in ("paragraph", "token"):
            raise ValueError(
                f"Invalid chunk_strategy: {chunk_strategy}. Must be 'paragraph' or 'token'."
            )
        if chunk_strategy == "token" and tokens_per_chunk <= 0:
            raise ValueError(
                "tokens_per_chunk must be positive when strategy is 'token'."
            )

        self.chunk_strategy = chunk_strategy
        self.tokens_per_chunk = tokens_per_chunk
        logger.info(
            f"Initialized SimpleDocumentProcessor with strategy: {self.chunk_strategy}, tokens_per_chunk: {self.tokens_per_chunk}"
        )
    ...
    async def chunk_document(
        ...
        | None = None,  # Add max_tokens_per_chunk if needed by token strategy
    ) -> list[Chunk]:
        """Splits document content based on the configured strategy."""

        # Use passed document_id and content directly
        logger.debug(
            f"Chunking document {document_id} using strategy: {self.chunk_strategy}"
        )
        chunks = []
        ...
        if not processed_content:
            logger.debug(
                f"Document {document_id} has no content after stripping whitespace, returning 0 chunks."
            )
            return []
        ...
        if self.chunk_strategy == "paragraph":
            ...
            for i, paragraph in enumerate(paragraphs):
                ...
                if paragraph_text:  # Ignore empty paragraphs
                    ...
                    logger.debug(
                        f"Created chunk {chunk_id} (UUID: {chunks[-1].id}) for doc {document_id}"
                    )
                    current_chunk_index += (
                        1  # Keep track if needed, though UUID is primary ID
                    )
            ...
            split_token_count = (
                ...
                else self.tokens_per_chunk
            )
            if split_token_count <= 0:
                logger.warning(
                    f"Invalid tokens_per_chunk ({split_token_count}) for doc {document_id}, falling back to paragraph strategy."
                )
        ...
        logger.info(
            f"Generated {len(chunks)} chunks for document {document_id} using {self.chunk_strategy} strategy."
        )
        return chunks
