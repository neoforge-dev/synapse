...
import re
from neo4j import AsyncDriver, AsyncGraphDatabase # <-- IMPORT AsyncDriver
...
class MemgraphRepository(GraphStore):
    ...
    def __init__(self, driver: AsyncDriver):
        ...
        self._driver = driver
        logger.info(f"MemgraphRepository initialized with neo4j AsyncDriver.")
    ...
    async def add_entity(self, entity: Entity):
        ...
        properties_map['id'] = entity.id # Ensure ID is part of the properties for MERGE/SET
    ...
    async def add_relationship(self, relationship: Relationship):
        ...
        try:
            result, summary, keys = await self._driver.execute_query(query_check, params_check)
            if result and result[0]:
                source_exists = result[0]['source_exists']
                target_exists = result[0]['target_exists']
                
                if not source_exists and not target_exists:
                    raise ValueError(f"Failed to add/update relationship {relationship.id}: Both source node {relationship.source_id} and target node {relationship.target_id} do not exist. Source or target node may not exist.")
                elif not source_exists:
                    raise ValueError(f"Failed to add/update relationship {relationship.id}: Source node {relationship.source_id} does not exist. Source or target node may not exist.")
                elif not target_exists:
                    raise ValueError(f"Failed to add/update relationship {relationship.id}: Target node {relationship.target_id} does not exist. Source or target node may not exist.")
            
            # If we get here, both nodes exist, so proceed with the relationship creation
            # Prepare properties map for SET clause (replaces existing properties)
            properties_map = {**relationship.properties}
            properties_map['id'] = relationship.id # Ensure ID is part of the properties for MERGE/SET
            properties_map['updated_at'] = datetime.now() # Add/update timestamp
            ...
            logger.error(f"Neo4j error adding relationship {relationship.id}: {e}", exc_info=True)
            raise ValueError(f"Failed to add/update relationship {relationship.id}: {e}. Source or target node may not exist.")
        except ValueError as e:
            # Re-raise ValueError exceptions (from our own checks)
            logger.error(f"Error adding relationship {relationship.id}: {e}", exc_info=True)
            raise
        except Exception as e:
            # Wrap other exceptions
            logger.error(f"Unexpected error adding relationship {relationship.id}: {e}", exc_info=True)
            raise ValueError(f"Failed to add/update relationship {relationship.id} due to an unexpected error: {e}")

    async def add_entities_and_relationships(self, entities: List[Entity], relationships: List[Relationship]):
        ...
        async with self._driver.session() as session:
            try:
                ...
                logger.info(f"Bulk added {len(entities)} entities and {len(relationships)} relationships.")

            except Exception as e:
                logger.error(f"Error during bulk operation: {e}. Transaction will be rolled back.", exc_info=True)
    ...
    def _map_node_to_entity(self, node_data: Any) -> Entity:
        ...
        if isinstance(node_data, Neo4jNode):
            ...
            created_at = _convert_neo4j_temporal_types(properties.pop('created_at', None))
            updated_at = _convert_neo4j_temporal_types(properties.pop('updated_at', None))
            # properties dict now contains only user-defined properties
            if node_id is None:
                 # Use Neo4j's internal element_id if app 'id' is missing
                logger.warning(f"Node {node_data.element_id} of type {node_type} is missing 'id' property! Using element_id.")
            ...
            node_type = data.get('type', 'Unknown') # Assuming 'type' alias was used in query
            if not node_type and 'labels' in data:
                 node_type = data['labels'][0] if data['labels'] else 'Unknown'
            properties = data.get('properties', {}) # Assuming 'properties' alias was used
            ...
            logger.error(f"Cannot map data of type {type(node_data)} to Entity: {node_data}")
            raise TypeError(f"Unsupported data type for node mapping: {type(node_data)}")
    ...
    async def get_entity_by_id(self, entity_id: str) -> Optional[Entity]:
        ...
        try:
            ...
            logger.error(f"Neo4jError retrieving entity {entity_id}: {e.message} (Code: {e.code})", exc_info=True)
            raise
        except Exception as e:
            logger.error(f"Unexpected error retrieving entity {entity_id}: {e}", exc_info=True)
            raise

    def _map_edge_to_relationship(self, edge_record: Union[Record, Neo4jRelationship]) -> Relationship:
        ...
        if isinstance(edge_record, Neo4jRelationship):
            ...
            source_id = str(rel_obj.start_node.get('id')) if rel_obj.start_node else None
            target_id = str(rel_obj.end_node.get('id')) if rel_obj.end_node else None
            created_at = _convert_neo4j_temporal_types(properties.pop('created_at', None))
            updated_at = _convert_neo4j_temporal_types(properties.pop('updated_at', None))
            if rel_id is None:
                 logger.warning(f"Relationship {rel_obj.element_id} of type {rel_type} is missing 'id' property! Using element_id.")
                 rel_id = rel_obj.element_id
            if source_id is None:
                logger.warning(f"Relationship {rel_id} is missing start node 'id' property!")
            if target_id is None:
                logger.warning(f"Relationship {rel_id} is missing end node 'id' property!")
            ...
            if isinstance(rel_data, Neo4jRelationship):
                ...
                properties = rel_data
                rel_id = str(properties.pop('id', data.get('relId', None))) # Check for alias 'relId'
                ...
                updated_at = _convert_neo4j_temporal_types(properties.pop('updated_at', None))
                 ...
                 raise TypeError(f"Unsupported Record structure for relationship mapping: {data}")
        else:
            logger.error(f"Cannot map data of type {type(edge_record)} to Relationship: {edge_record}")
            raise TypeError(f"Unsupported data type for relationship mapping: {type(edge_record)}")
    ...
    async def get_neighbors(self, entity_id: str, relationship_types: Optional[List[str]] = None, direction: str = "both") -> Tuple[List[Entity], List[Relationship]]:
        ...
        if relationship_types:
            ...
            safe_types = [rt.replace("`", "").replace(":", "") for rt in relationship_types]
        ...
        try:
            ...
            logger.error(f"Error getting neighbors for entity {entity_id}: {e}", exc_info=True)
            return [], [] # Return empty lists on error

    async def search_entities_by_properties(self, properties: Dict[str, Any], limit: Optional[int] = None) -> List[Entity]:
        ...
        try:
            ...
            logger.error(f"Error searching entities by properties {properties}: {e}", exc_info=True)
    ...
    async def close(self) -> None:
        ...
        if self._driver:
            try:
                ...
                logger.info(f"Neo4j AsyncDriver closed.")
            except Exception as e:
    ...
    async def delete_document(self, document_id: str) -> bool:
        ...
        logger.debug(f"[Delete Check] For ID '{document_id}', get_entity_by_id returned: {existing_doc!r} (Type: {type(existing_doc)})")
        
        if not existing_doc or existing_doc.type != "Document":
            logger.warning(f"Attempted to delete non-existent document {document_id}. Returning False based on check.")
        ...
        try:
            results, summary, keys = await self._driver.execute_query(query, {"doc_id": document_id})
            nodes_deleted = summary.counters.nodes_deleted
            if nodes_deleted > 0:
                 logger.info(f"Deletion query successful for {document_id}, nodes deleted: {nodes_deleted}. Returning True.")
                 ...
                 logger.warning(f"Document {document_id} existed but deletion query reported 0 nodes deleted. Returning False.")
                 return False # Return False if deletion query deleted nothing (unexpected)

        except Exception as e:
            logger.error(f"Error during deletion query for document {document_id}: {e}", exc_info=True)
