...
from graph_rag.api.auth.enterprise_models import Tenant, TenantUser
...
class SecureTenantRepository(ABC):
    ...
    @abstractmethod
    async def read(self, entity_id: str) -> Optional[Dict[str, Any]]:
...
class TenantSecurityBoundary:
    ...
    def get_security_violations(self, tenant_id: Optional[UUID] = None) -> List[Dict[str, Any]]:
