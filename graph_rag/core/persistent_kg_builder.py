...
from graph_rag.data_stores.memgraph_store import MemgraphStore # Use our MemgraphStore
...
class PersistentKnowledgeGraphBuilder(KnowledgeGraphBuilder):
    ...
    def __init__(self, graph_store: MemgraphStore):
    ...
    async def add_document(self, doc: DocumentData) -> None:
        ...
        try:
            ...
            doc_props = {
                ...
                "created_at": datetime.utcnow().isoformat(), # Add timestamp on creation
                "updated_at": datetime.utcnow().isoformat(),
            }
            await self.graph_store.add_node(label="Document", properties=doc_props)
        except Exception as e:
            logger.error(f"Failed to add document {doc.id} to graph store: {e}", exc_info=True)
            raise # Re-raise to signal failure

    async def add_chunk(self, chunk: ChunkData) -> None:
        logger.debug(f"Adding/merging chunk node {chunk.id} for doc {chunk.document_id} to persistent store.")
        try:
            chunk_props = {
                "id": chunk.id,
                "text": chunk.text,
            ...
            if chunk.document_id:
                ...
                logger.debug(f"Linked Document {chunk.document_id} -[:CONTAINS]-> Chunk {chunk.id}")
        except Exception as e:
            logger.error(f"Failed to add chunk {chunk.id} or link to document: {e}", exc_info=True)
            raise

    async def add_entity(self, entity: ExtractedEntity) -> None:
        logger.debug(f"Adding/merging entity node {entity.id} ({entity.label}) to persistent store.")
        try:
            entity_props = {
                "id": entity.id,
                "label": entity.label,
    ...
    async def add_relationship(self, relationship: ExtractedRelationship) -> None:
        logger.debug(f"Adding relationship {relationship.source_entity_id} -[:{relationship.label}]-> {relationship.target_entity_id}")
        try:
            rel_props = {
                "created_at": datetime.utcnow().isoformat(),
                "updated_at": datetime.utcnow().isoformat(),
            ...
            logger.error(f"Failed to add relationship {relationship.label} between {relationship.source_entity_id} and {relationship.target_entity_id}: {e}", exc_info=True)
            raise
    ...
    async def link_chunk_to_entities(self, chunk_id: str, entity_ids: List[str]) -> None:
        ...
        link_props = {"created_at": datetime.utcnow().isoformat()}
        errors = []
        for entity_id in entity_ids:
            try:
                ...
                logger.debug(f"Linked Chunk {chunk_id} -[:MENTIONS]-> Entity {entity_id}")
            except Exception as e:
                # Log error and continue trying to link other entities
                logger.error(f"Failed to link chunk {chunk_id} to entity {entity_id}: {e}", exc_info=False)
                errors.append(entity_id)
        
        if errors:
            # Optionally raise a summary error or just log
            logger.warning(f"Failed to link chunk {chunk_id} to some entities: {errors}")
