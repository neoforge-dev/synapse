...
class MockGraphStore(GraphStore):
    ...
    def add_entity(self, entity: Entity):
        ...
        logger.debug(
            f"MockGraphStore adding entity: {entity.type} - {entity_name} (ID: {entity.id})"
        )
    ...
    def add_relationship(self, relationship: Relationship):
        ...
        logger.debug(
            f"MockGraphStore adding relationship: {source_name} -[{relationship.type}]-> {target_name}"
        )
    ...
    def add_entities_and_relationships(
        ...
        logger.info(
            f"MockGraphStore: Bulk adding {len(entities)} entities and {len(relationships)} relationships."
        )
    ...
    def get_neighbors(
        ...
        logger.debug(
            f"MockGraphStore: Getting neighbors for {entity_id} (types: {relationship_types}, direction: {direction})"
        )
        ...
        for rel in self.relationships.values():
            ...
            if direction in ["outgoing", "both"] and rel.source.id == entity_id:
                ...
                if relationship_types is None or rel.type in relationship_types:
                    ...
                    neighbor_id = (
                        rel.source.id
                    )  # The neighbor is the source in this case
        ...
        logger.debug(
            f"MockGraphStore: Found {len(neighbor_entities)} neighbors and {len(connecting_relationships)} relationships for {entity_id}."
        )
    ...
    def search_entities_by_properties(
        ...
        logger.debug(
            f"MockGraphStore: Found {len(matches)} entities matching properties."
        )
        return matches
    ...
    def delete_document(self, document_id: str) -> bool:
        ...
        if document_id in self.entities:
            ...
            logger.warning(
                f"MockGraphStore: Document {document_id} not found for deletion."
            )
            return False
    ...
    def add_chunk(self, chunk):
        ...
        logger.info(
            f"MockGraphStore: Adding chunk {chunk.id} for document {chunk.document_id}"
        )
